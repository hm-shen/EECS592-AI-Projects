...
public class KnightsTour {
    ...
    static boolean isAlive(Chessboard board, int row, int col, int ST) {
        ...
        if (ST == 0 || ST == 1 || ST == 2 || ST == 4) {
            int deg = board.getDynamicDeg(row, col);
    ...
    static ArrayList<int[]> reorderNbrs(Chessboard board, ArrayList<int[]> candNodes, int ST) {
        if (ST == 0) {
            ...
            candNodes = sortAccordDynDeg(board, candNodes);
        } else if (ST == 3) {
            candNodes = sortAccordFixDeg(board, candNodes);
        } else if (ST == 4) {
    ...
    public static void main(String[] args)
    {
        ...
        numOfKTfound = solveKT(board, startNode[0], startNode[1], ST, numIter,
                outputPath);
